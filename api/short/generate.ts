...
// Short-tier style bible — same shape as trailer but with 4-6
// characters to support the longer runtime.
const STYLE_BIBLE_PROMPT = `You are the director on a short film. Before any shot is drawn or any clip is rendered, write a STYLE BIBLE as strict JSON. Every descriptor must be specific, visual, and usable by an image model.

{
  "tone_references": ["<Film 1>", "<Film 2>", "<Film 3 optional>"],
  "palette": "<6-12 word palette description>",
  "camera_philosophy": "<1 sentence — lens, framing bias, motion, light>",
  "locations": [ "<hook-description>", "<another>", "<another>" ],
  "characters": [
    {
      "name": "<character name>",
      "role": "<protagonist | antagonist | supporting>",
      "visual_anchor": "<20-25 words of strictly visual detail — age, build, hair, wardrobe, distinctive prop, vibe. Quoted verbatim in every shot.>"
    }
  ]
}
...
- 4-6 characters if the story supports it; protagonist first.
- visual_anchor must be strictly visual — what a camera sees.
- Output ONLY the JSON object. No markdown fences, no preamble.`;

const STORYBOARD_PROMPT_PROMPT = `Given a film title, synopsis, treatment, and STYLE BIBLE, write 12 distinct storyboard frame descriptions as a JSON array of strings.

Each description is a 40-80 word cinematic visual prompt suitable for an AI image generator. Cover the full arc: opening image, character intro, inciting incident, rising action, midpoint, dark moment, climax, final image, plus key supporting scenes.

MUST DO for every frame:
- If a character from the style bible appears, quote their visual_anchor VERBATIM. Never paraphrase.
- Name the location from the bible's locations list.
- State shot type, lighting, and palette from the bible.
- Reference the tone by naming one of the tone_reference films.

CRITICAL: Output ONLY the JSON array of 12 strings. No preamble, no markdown fences.`;

const VIDEO_CLIP_PROMPT = `Given a film title, synopsis, treatment, and STYLE BIBLE, write 8 distinct 8-second video clip prompts as a JSON array of strings.

These 8 shots compose a ~64-second short. Pace them across the arc: opening shot, character moments, inciting beat, action/tension, midpoint twist, quiet moment, climax, final image.

MUST DO for every clip:
- 60-100 words. Cinematic, present-tense, motion-aware.
- If a character from the style bible appears, quote their visual_anchor VERBATIM. Non-negotiable for continuity.
- Name the location from the bible.
- Open with shot type + camera movement.
- Name the lighting + palette from the bible.
- Reference the tone by naming one of the tone_reference films.
- End on an evocative beat, not a cut instruction.

MUST NOT:
- Paraphrase visual_anchors — quote them.
- Invent characters not in the bible.
- Use generic words like "vibrant" — use bible palette.
...
export default async function handler(
  ...
  try {
    ...
    // ── 2. Style Bible ───────────────────────────────────────────
    //
    // Hands every visual step a shared vocabulary so character
    // wardrobes, locations, palette, and tone stay consistent across
    // 8 clips + 12 storyboard frames instead of drifting per call.
    let styleBible: {
    ...
    try {
      ...
      await attach('director', 'text',
        'data:application/json;charset=utf-8,' + encodeURIComponent(JSON.stringify(styleBible)),
        'grok-3-mini', 'style bible', 0.01, 'director.style_bible');
    } catch (err) {
      console.error('[short] style bible failed — falling back to treatment-only:', err);
      bibleContext = '';
    }
    ...
    // 3. Poster — bible-aware: protagonist anchor + palette + tone ref
    try {
      ...
      const posterTone = styleBible?.tone_references?.[0]
        ...
        : '';
      const posterPrompt = `Cinematic movie poster for the short film "${offer.title}". ${offer.synopsis}${posterAnchor}${posterPalette}${posterTone} Portrait orientation (2:3), bold title typography, dramatic lighting, atmospheric mood. Award-caliber movie poster.`;
    ...
    try {
      const storyboardContext =
        `${context}\n\nTreatment:\n${treatment.slice(0, 5000)}${bibleContext}`;
      const raw = await xaiChat(xaiKey, STORYBOARD_PROMPT_PROMPT, storyboardContext, 2000);
    ...
    try {
      const clipContext =
        `${context}\n\nTreatment:\n${treatment.slice(0, 5000)}${bibleContext}`;
      const raw = await xaiChat(xaiKey, VIDEO_CLIP_PROMPT, clipContext, 2200);
