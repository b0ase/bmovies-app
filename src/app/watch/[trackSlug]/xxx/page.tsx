...
export default function XXXWatchPage() {
  ...
  useEffect(() => {
    ...
    const hasToken = document.cookie.includes(`npgx_xxx_${trackSlug}=owned`)
    const hasLocal = localStorage.getItem(`npgx-xxx-${trackSlug}`) === 'paid'
  ...
  const handlePurchase = async () => {
    ...
    try {
      const res = await fetch(`/api/auth/handcash/pay`, {
        ...
        body: JSON.stringify({ amount: PRICE, product: `xxx-${trackSlug}`, description: `XXX Music Video — ${trackSlug}` }),
      })
      if (res.ok) {
        const data = await res.json()
        if (data.success || data.redirectUrl) {
          // Payment succeeded — set cookie
          document.cookie = `npgx_xxx_${trackSlug}=owned; path=/; max-age=${365 * 24 * 60 * 60}`
          setPaid(true)
          setTimeout(() => {
            playStartRef.current = Date.now()
            setIsPlaying(true)
    ...
    localStorage.setItem(`npgx-xxx-${trackSlug}`, 'paid')
    document.cookie = `npgx_xxx_${trackSlug}=owned; path=/; max-age=${365 * 24 * 60 * 60}`
    setPaid(true)
    setTimeout(() => {
      playStartRef.current = Date.now()
    ...
    return (
      <div className="fixed inset-0 bg-black z-[200] flex items-center justify-center">
        <div className="max-w-md mx-auto text-center px-6">
          <div className="font-[family-name:var(--font-brand)] text-6xl text-red-600 tracking-wider mb-4">XXX</div>
          ...
          <div className="border border-red-500/30 bg-red-600/5 p-6 mb-6">
            <div className="font-[family-name:var(--font-brand)] text-xs text-red-400 uppercase tracking-widest mb-2">Buy $XXX Token</div>
  ...
  return (
    <div className="fixed inset-0 bg-black z-[200]" onClick={() => isPlaying ? (audioRef.current?.paused ? audioRef.current?.play().catch(()=>{}) : audioRef.current?.pause()) : handleStart()}>
      ...
      <div className="absolute top-4 left-4 z-20">
        <div className="font-[family-name:var(--font-brand)] text-red-600 text-xs tracking-[0.3em] opacity-30">XXX</div>
      </div>
