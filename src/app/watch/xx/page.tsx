...
async function getXXTracks(): Promise<XXTrack[]> {
  const tracks: XXTrack[] = []

  for (const album of ALBUMS) {
    for (const track of album.tracks) {
      if (track.status !== 'recorded') continue

      // Check for XX subfolder
      const xxDir = join(process.cwd(), 'public', 'music-videos', `${track.slug}-1`, `${track.slug}-xx`)
      let clipCount = 0
      try {
        const files = await readdir(xxDir)
        clipCount = files.filter(f => f.endsWith('.mp4')).length
      } catch {
        continue // No XX folder — skip
      }
...
export default async function XXIndexPage() {
  const tracks = await getXXTracks()

  return (
    <div className="min-h-screen bg-black text-white">
      <div className="max-w-4xl mx-auto px-4 py-12">
        <div className="flex items-center gap-4 mb-2">
          <h1 className="font-[family-name:var(--font-brand)] text-5xl text-red-600 tracking-wider">XX</h1>
          <div className="h-px flex-1 bg-red-600/20" />
        </div>
        <p className="text-white/30 text-sm mb-8">Explicit music videos — $49 per video • Age verification required</p>

        {tracks.length === 0 ? (
          <div className="border border-white/5 bg-white/[0.02] p-12 text-center">
            <div className="text-white/20 font-[family-name:var(--font-brand)] text-sm">No XX content available yet</div>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {tracks.map(track => (
              <Link key={track.slug} href={`/watch/${track.slug}/xx`}
